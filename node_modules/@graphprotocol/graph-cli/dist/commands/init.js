...
import { createIpfsClient, getMinStartBlock, loadManifestYaml, loadSubgraphSchemaFromIPFS, validateSubgraphNetworkMatch, } from '../utils.js';
...
export default class InitCommand extends Command {
    ...
    async run() {
        const { args: { argSubgraphName, argDirectory }, flags, } = await this.parse(InitCommand);
        const subgraphName = formatSubgraphName(argSubgraphName ?? '');
        const directory = argDirectory ?? '';
        const { protocol, node: nodeFlag, 'from-contract': fromContract, 'from-subgraph': fromSubgraph, 'contract-name': contractName, 'from-example': fromExample, 'index-events': indexEvents, 'skip-install': skipInstall, 'skip-git': skipGit, ipfs, network, abi: abiPath, 'start-block': startBlock, spkg: spkgPath, } = flags;
        ...
        if ((fromContract || spkgPath || fromSubgraph) &&
            protocol &&
            subgraphName &&
            directory &&
            network &&
...
async function processInitForm({ abi: initAbi, abiPath: initAbiPath, directory: initDirectory, source: initContract, indexEvents: initIndexEvents, fromExample: initFromExample, subgraphName: initSubgraphName, contractName: initContractName, startBlock: initStartBlock, spkgPath: initSpkgPath, ipfsUrl, }) {
    try {
        const registry = await loadRegistry();
        const contractService = new ContractService(registry);
